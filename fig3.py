...
class MakeDistro_dydt:
    ...
    Here, I'm using ki and kd to indicate the infection and decay rate constants
    (to avoid using confusing ourselves with two things called gamma!).
    get_ebar is a function that returns the current mean value of the distribution
    given ki and C, where C is the cumulative infection pressure.
    """
...
dax.text(times[-1] + 1.7, 1. - ys[0, -1], r'$\beta I S^{:g}$'.format(power), va='center')
power = 1.5
ys = get_power_results(power, ki, kd, times, xI0)
...
gammacurves = []
k = 0.25
c = kax.plot([phis[0], phis[-1]], [1. / k]*2, '-', label='Gamma: $k$={:g}'.format(k))[0]
gammacurves += [c]
k = 0.5
c = kax.plot([phis[0], phis[-1]], [1. / k]*2, '-', label='Gamma: $k$={:g}'.format(k))[0]
gammacurves += [c]
...
for curve in betacurves:
    y -= 1.
    add_entry(1.7, y, curve.get_label(), curve.get_linestyle(), curve.get_color(), lw=2.0)
y = 1.
for curve in gammacurves:
    y -= 1.
    add_entry(0.8, y, curve.get_label(), curve.get_linestyle(), curve.get_color(), lw=2.0)
