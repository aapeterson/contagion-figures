...
class MakeDistro_dydt:
    ...
    Here, I'm using ki and kd to indicate the infection and decay rate
    constants (to avoid using confusing ourselves with two things called
    gamma!).  get_ebar is a function that returns the current mean value of
    the distribution given ki and C, where C is the cumulative infection
    pressure.
    """
...
final_phis = {}  # For use in order-parameter plot.

# Power models.
for power in [1., 1.5, 2., 3., 5.]:
    ys = get_power_results(power, ki, kd, times, xI0)
    dax.plot(times, 1. - ys[0], lw=6., color='0.8')
...
kax.set_ylabel('$\mathcal{K}(t)$, general order parameter')
kax.set_xlabel('$\phi(t)$, progress variable')
